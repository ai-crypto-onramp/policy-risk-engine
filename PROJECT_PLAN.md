...
**Tasks:**
- [ ] Choose a migration tool (e.g. `golang-migrate` or `goose`) and add it to the Makefile.
- [ ] Create `migrations/` directory with ordered up/down SQL files.
- [ ] Define `policies` table (id, scope, active_version FK, created_at).
- [ ] Define `policy_versions` table (id, policy_id, version, rego_hash, rego_source, created_at, created_by).
- [ ] Define `policy_decisions` append-only audit table (decision_id, policy_version, request_hash, decision, reasons[], applied_rules[], score, signature, created_at).
- [ ] Define `whitelist_addresses` table (user_id, chain, address, label, verified_at, status).
- [ ] Define `review_queue` table (decision_id, tx_id, status, assigned_to, resolved_at, resolution).
- [ ] Add DB connection bootstrap in `cmd/policy-engine` using `DB_URL`.
- [ ] Add a `make migrate-up` / `make migrate-down` target and a docker-compose snippet for local Postgres.
...
**Tasks:**
- [ ] Add `github.com/open-policy-agent/opa` dependency and pin version.
- [ ] Implement a `policy/engine` package wrapping `opa.Context` with a swappable decision engine.
- [ ] Support loading a bundle from disk (`policies/`) and from `OPA_BUNDLE_URL` (bundle service).
- [ ] Compute and store `rego_hash` (SHA-256 of canonical Rego source) per bundle.
- [ ] Seed `policies/decisions.rego` and `policies/caps.yaml` from README examples.
- [ ] Add `OPA_BUNDLE_POLL_INTERVAL` polling loop that downloads and stages a new bundle without activating it.
- [ ] Unit test: load a known Rego bundle and assert deterministic `allow` / `manual_review` outcomes for fixture inputs.
...
- [ ] Implement REST handler with the exact JSON contract from README (decision, reasons, applied_rules, policy_version, score, decision_id).
- [ ] Implement gRPC server mirroring the same logic.
- [ ] Generate `decision_id` as `dec_<ULID>` and stamp the active `policy_version` on every response.
- [ ] Wire mTLS for gRPC (`MTLS_CA_CERT`) and scope-checked bearer auth for REST admin paths.
- [ ] Add request validation (required fields, amount > 0, currency in allowlist).
- [ ] Add structured logging + Prometheus metrics for decision counts by outcome and latency histogram.
...
**Tasks:**
- [ ] Add a Redis client wired from `REDIS_URL` with health check.
- [ ] Implement `vel:{user_id}:{window}` counters using atomic `INCR` + `EXPIRE`.
- [ ] Support optional per-asset / per-rail buckets `vel:{user_id}:{asset}:{window}`.
- [ ] Read window lengths from `VELOCITY_WINDOW_MIN_SEC` / `_HOUR_SEC` / `_DAY_SEC`.
- [ ] Expose a `Counter.Increment` / `Counter.Rollback` API for the compensating decrement path.
- [ ] Inject velocity counters into the OPA input document under `input.velocity`.
- [ ] Unit + integration tests with a real Redis (testcontainers or miniredis).
...
**Tasks:**
- [ ] Load tier / asset / rail cap rules from `policies/caps.yaml` into OPA data documents.
- [ ] Add FX conversion step using latest quote snapshot (input `fx_rate_to_usd` or fetched externally).
- [ ] Implement `on_exceed: deny` and `on_exceed: manual_review` branches.
- [ ] Apply per-tier overrides resolved by Rego data keyed on `user_tier`.
- [ ] Add `DEFAULT_DAILY_CAP_USD` and `DEFAULT_TX_CAP_USD` fallbacks when no rule matches.
- [ ] Add near-cap threshold (e.g. 90% of cap) routing to `manual_review`.
- [ ] Tests: each tier × asset × rail combination produces the expected decision and `applied_rules` entry.
...
**Tasks:**
- [ ] Implement `POST /v1/policy/whitelist` and `GET /v1/policy/whitelist/:user_id` REST handlers.
- [ ] Implement whitelist verification flow (status transitions: `pending` → `verified` after micro-transfer / signed message confirmation).
- [ ] Inject whitelist into OPA input as `data.whitelist[user_id]` map.
- [ ] Deny non-whitelisted destinations unless a one-time override policy routes to `manual_review`.
- [ ] Validate JWT session from Identity & Auth on the REST path; enforce scope checks for admin endpoints.
- [ ] Require `2fa_passed=true` when `amount_usd >= HIGH_VALUE_2FA_THRESHOLD_USD`.
- [ ] Tests: whitelisted / non-whitelisted / unverified / override paths; 2FA required and bypassed cases.
...
**Tasks:**
- [ ] On `manual_review`, insert a `review_queue` row with `status=pending`, `tx_id`, `decision_id`.
- [ ] Implement `POST /v1/policy/review/:decision_id/resolve` (reviewer auth scope required).
- [ ] On resolve, update `review_queue.status=resolved`, set `resolution`, `resolved_at`, `assigned_to`.
- [ ] Emit a follow-up signed `PolicyDecisionRecord` with the final decision.
- [ ] Notify the Transaction Orchestrator of resolution (webhook or poll endpoint) so the saga can resume.
- [ ] Tests: pending → resolved lifecycle; unauthorized reviewer rejected; double-resolve rejected.
...
**Tasks:**
- [ ] Implement `GET /v1/policy/rules`, `POST /v1/policy/rules` (with `activate=true|false`), `GET /v1/policy/rules/:version`.
- [ ] On publish: create a new immutable `policy_versions` row; never mutate existing rows.
...
- [ ] Keep a versioned history of loaded engines for audit replay.
- [ ] Tests: publish → activate → evaluate → rollback to previous version reproduces prior decision.
...
**Tasks:**
- [ ] Load the service identity signing key from `SERVICE_IDENTITY_KEY` (Ed25519).
- [ ] Compute canonical-JSON payload + policy version hash; sign with Ed25519.
- [ ] Persist the signed record to `policy_decisions` in the same DB transaction as the response write.
- [ ] Publish `PolicyDecisionRecord` event to `audit-event-log` via `AUDIT_EVENT_LOG_URL` (at-least-once with idempotency key = `decision_id`).
- [ ] Ensure replay-safety: re-evaluating the same payload against the same version reproduces the same record hash.
- [ ] Tests: signing determinism, audit emission on deny/manual_review/allow, DB-transaction atomicity (rollback emits nothing).
...
- [ ] Reach ≥80% unit test coverage across Go packages.
- [ ] Add integration tests (testcontainers Postgres + Redis) for the full evaluate path.
- [ ] Add golangci-lint to CI and resolve findings.
- [ ] Finalize `Dockerfile` (multi-stage, distroless or scratch runtime, non-root user).
- [ ] Add `docker-compose.yml` for local Postgres + Redis + OPA bundle mock + the service.
- [ ] Add load test harness asserting p99 < 50ms on the gRPC evaluate path.
- [ ] Update README with the finalized dev workflow.
